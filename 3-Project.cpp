...
Display( )
{
	...
	if (TextureOn && DistortionOn)
	{
		glEnable(GL_TEXTURE_2D);
		glBindTexture(GL_TEXTURE_2D, shallow_water_t);
		...
		float s, t, real_s, real_t;
		for (int i = 1; i <= 10; i++) //Rows
		{
			for (int k = 1; k <= 10; k++) //Columns
			{
				real_s = (float)(i - 1) / 10.f;
				real_t = (float)(k - 1) / 10.f;
				s = cos(real_s + sin(Time*real_s)) + real_s;
				t = sin(real_t + cos(Time*real_t)) + real_t;
				glTexCoord2f(s, t);
				glVertex2f(i - 1, k - 1);
...
InitLists( )
{
	float dx = PLANESIZE / 2.f;
	float dy = PLANESIZE / 2.f;
	glutSetWindow( MainWindow );
...
InitTextures( )
{
	...
	unsigned char * shallow_water_a = BmpToTexture("shallow-water.bmp", &width, &height);

	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glGenTextures(1, &shallow_water_t);

	glBindTexture(GL_TEXTURE_2D, shallow_water_t); //Make shallow water texture current
	...
	glTexImage2D(GL_TEXTURE_2D, level, ncomps, width, height, border, GL_RGB, GL_UNSIGNED_BYTE, shallow_water_a);
}
